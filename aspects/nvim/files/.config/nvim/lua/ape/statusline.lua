...
local update_statusline = function(default, action)
  ...
  elseif filetype == 'qf' then
    if action == 'blur' then
      result = '%{v:lua.ape.statusline.gutterpadding()}'
        .. ' '
        .. ' '
        .. ' '
        .. ' '
      ...
      result = vim.g.ApeQuickfixStatusline or ''
    end
  else
    result = 1
  end
...
statusline.blur_statusline = function()
  ...
  local blurred = '%{v:lua.ape.statusline.gutterpadding()}'
...
statusline.set = function()
  ...
  vim.opt.statusline = ''
    ...
    .. '%{v:lua.ape.statusline.lhs()}' -- Red/green/orange modified/activity status.
    ...
    .. '%{v:lua.ape.statusline.fileprefix()}' -- Relative path to file's directory.
    ...
    .. '%{v:lua.ape.statusline.filetype()}' -- Filetype (not using %Y because I don't want caps).
    .. '%{v:lua.ape.statusline.fileencoding()}' -- File-encoding if not UTF-8.
    ...
    .. '%{v:lua.ape.statusline.rhs()}' -- Line/column info.
    .. '%*' -- Reset highlight group.
end
