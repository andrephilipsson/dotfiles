...
local command = function(name, repl, opts)
  ...
  if repl_type == 'function' then
    local key = ape.util.get_key_for_fn(repl, ape.g.command_callbacks)
    ape.g.command_callbacks[key] = repl
    repl = 'lua ape.g.command_callbacks.' .. key .. '()'
