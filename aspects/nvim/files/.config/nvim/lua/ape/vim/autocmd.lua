...
local autocmd = function(name, pattern, cmd, opts)
  ...
  if cmd_type == 'function' then
    local key = ape.util.get_key_for_fn(cmd, ape.g.autocommand_callbacks)
    ape.g.autocommand_callbacks[key] = cmd
    cmd = 'lua ape.g.autocommand_callbacks.' .. key .. '()'
