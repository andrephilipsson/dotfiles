...
__APE[ITALIC_ON]=$'\e[3m'
__APE[ITALIC_OFF]=$'\e[23m'
...
zstyle ':completion:*:descriptions' format %F{default}%B%{$__APE[ITALIC_ON]%}--- %d ---%{$__APE[ITALIC_OFF]%}%b%f
...
source $HOME/.config/zsh/common
source $HOME/.config/zsh/color
source $HOME/.config/zsh/exports
source $HOME/.config/zsh/functions
source $HOME/.config/zsh/hash
source $HOME/.config/zsh/vars
...
# test -e "$HOME/.config/zsh/skim/shell/key-bindings.zsh" && source "$HOME/.config/zsh/skim/shell/key-bindings.zsh"
test -e "$HOME/.config/zsh/skim/shell/completion.zsh" && source "$HOME/.config/zsh/skim/shell/completion.zsh"
...
function -update-title-precmd() {
  ...
  setopt EXTENDED_GLOB
  setopt KSH_GLOB
  if [[ __APE[HISTCMD_LOCAL] -eq 0 ]]; then
...
function -update-title-preexec() {
  emulate -L zsh
  __APE[HISTCMD_LOCAL]=$((++__APE[HISTCMD_LOCAL]))
  if [ -n "$TMUX" ]; then
...
function -update-rprompt() {
  ...
  if [ $ZSH_START_TIME ]; then
    ...
    export RPROMPT="%F{cyan}%{$__APE[ITALIC_ON]%}${ELAPSED}%{$__APE[ITALIC_OFF]%}%f $RPROMPT_BASE"
...
HOST_RC=$HOME/.config/zsh/host/$(hostname -s | tr '[:upper:]' '[:lower:]')
